# Cristofer Amachi Cervantes
# CSE 111

# Exceeded the requirements by adding separate variables
# for the calculations. These are the following:
...
def pressure_gain_from_water_height(height):
    result = (WATER_DENSITY * EARTH_ACCELERATION_OF_GRAVITY * height) / 1000
    return result
...
def reynolds_number(hydraulic_diameter, fluid_velocity):
    result = (WATER_DENSITY * hydraulic_diameter * fluid_velocity) / WATER_DYNAMIC_VISCOSITY
    return result

def pressure_loss_from_pipe_reduction(larger_diameter, fluid_velocity, reynolds_number, smaller_diameter):
    k_constant = (0.1 + 50 / reynolds_number) * ((larger_diameter / smaller_diameter) ** 4 - 1)
    result = (-k_constant * WATER_DENSITY * fluid_velocity ** 2) / 2000
    return result
